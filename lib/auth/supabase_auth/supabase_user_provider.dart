...
class YekjaSupabaseUser extends BaseAuthUser {
  YekjaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> yekjaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = YekjaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
