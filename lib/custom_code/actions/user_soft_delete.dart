...
Future<bool> userSoftDelete() async {
  ...
  final String tempPassword =
      'LOCK_${timestamp}_${userId}'; // Ensures strong lock

  try {
    // 1. SOFT DELETE PROFILE (Trigger the DB/Archiving cascade)
    // The BEFORE DELETE trigger runs the dynamic archiving function here.
    await supabase.from('user_ext').delete().eq('id', userId);
    print("1. DB soft-delete and archiving executed.");

    // 2. SABOTAGE & LOCK (Call Edge Function securely)
    final response = await supabase.functions.invoke(
      'soft-delete-user', // The name of your deployed Edge Function
      body: {
        'userId': userId,
    ...
    print("Error during complete soft delete process: $error");
    return false;
  }
}
